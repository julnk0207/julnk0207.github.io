...
description: "벡터화는 ’벡터로 만든다’는 말이다. 데이터를 벡터로 만들면 반복문을 돌면서 계산할 필요 없이 행렬곱, 벡터곱 등을 이용해 편하게 계산할 수 있다. 예를 들어 z = wTx + b ( w ∈ Rnx , x ∈ Rnx ) 와 같은 선형회귀 식을 반복문을 이용해 계산하면 다음과 같다."
date: 2023-03-03
...
벡터화는 ’벡터로 만든다’는 말이다. 데이터를 벡터로 만들면 반복문을 돌면서 계산할 필요 없이 행렬곱, 벡터곱 등을 이용해 편하게 계산할 수 있다. 예를 들어 *z* = *wTx* + *b* (*w* ∈ *Rnx*, *x* ∈ *Rnx*) 와 같은 선형회귀 식을 반복문을 이용해 계산하면 다음과 같다.
...
`np.dot(w, x)` 부분이 *wTx* 를 계산하는 부분이다.
...
행렬 X는 (*nx*, *m*) 의 모양을 가지고 있다. 이 행렬을 *wT*와 곱하고, b로 이루어진 (1, *m*) 형태의 벡터와 더한다.
...
log *P*(*y*|*x*) = log *ŷy*(1 − *ŷ*)(1 − *y*) = *y*log *ŷ* + (1 − *y*)log (1 − *ŷ*) = −*L*(*ŷ*, *y*)
