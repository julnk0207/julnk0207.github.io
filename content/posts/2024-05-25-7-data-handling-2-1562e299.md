---
title: "[Python for ML] 7. Data handling - 2"
description: "Pandas(판다스)는 구조화된 데이터를 처리하는 파이썬 라이브러리이다. 쉽게 생각해서 파이선계의 엑셀이라고 보면 된다. 넘파이의 다양한 기능도 포함하고 있다."
date: 2024-05-25
...
`loc`은 인덱스의 이름을 받아서 반환한다. 반면, `iloc`은 인덱스의 위치를 받아서 반환한다. 다음 예시를 보면 차이가 좀 더 잘 보일 것이다.
...
삭제를 하더라도 기존 데이터는 변하지 않는다. 기존 데이터도 바뀌기를 원하면 `inplace=True`로 설정해야 한다.
