...
description: "$$MSE(\\mathbf{X}, h {\\theta}) = \\frac{1}{m} \\displaystyle \\sum {i=1}^{m}{(\\mathbf{\\theta}^T \\mathbf{x}^{(i)} - y^{(i)})^2}$$"
date: 2024-11-06
category: "Book"
...
비용함수를 최소화하는 *θ* 를 찾기 위한 해석적인 방법으로 정규방정식이 있다. 식은 다음과 같다.
