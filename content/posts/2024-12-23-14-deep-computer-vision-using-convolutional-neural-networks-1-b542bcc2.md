...
description: "The most important component of Convolutional Neural Network(CNN) is a convolutional layer. A neuron in the first convolutional layer is only connected to the pixel in…"
date: 2024-12-23
category: "Book"
...
A neuron at row i, column j in a layer is connected to the prior layer’s i to *i* + *fh* − 1 rows and j to *j* + *fw* − 1 columns outputs.
...
When using a stride, neuron *i*, *j* of upper layer is connected to the lower layer’s

*i* × *sh* to *i* × *sh* + *fh* − 1 rows,

*j* × *sw* to *j* × *sw* + *fw* − 1 columns (*sh* is a height of a stride, *sw* is a width of a stride).
...
A neuron*i*, *j* in a feature map *k* of CNN layer *l* are connected to the outputs of

*i* × *sh* to *i* × *sh* + *fh* − 1 rows,

*j* × *sw* to *j* × *sw* + *fw* − 1 columns
...
Conv2D layer does not use zero padding(

```
padding='valid'
```

). If you want to maintain the size, set

```
padding='same'
```

. However, stride 2+ cannot maintain the size.
