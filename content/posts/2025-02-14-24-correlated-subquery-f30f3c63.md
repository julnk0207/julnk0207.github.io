...
description: "To check whether data exists or not while using subquery, use EXISTS."
date: 2025-02-14
category: "Book"
...
It will be easy to understand that subquery is executed row by row whenever a row is retrieved using a `WHERE` clause of `UPDATE`. `EXISTS` returns True when row is returned by satisfying the condition(`WHERE` of subquery).. For example, the first row of table sample551 will not be changed because `EXISTS` will return False, and `EXISTS` returns False because the `WHERE` of subquery is not satisfied(1 ≠ 3 and 5).
...
The subquery that parent command and subquery correlates specific relationship is called correlated subquery. The command used above is an example of correlated subquery because subquery uses column of parent for comparison.
