...
description: "색상 분할: 동일한 색상을 가진 픽셀을 같은 세그먼트에 할당. (e.g. 인공위성 사진에서 한 지역을 산림 면적을 측정.) 시맨틱 분할: 동일한 종류의 물체에 속한 모든 픽셀을 같은 세그먼트에 할당. (e.g. 보행자 식별 프로그램) 인스턴스 분할: 개별 객체에 속한 모든 픽셀을 같은 세그먼트에 할당. (e.g.…"
date: 2024-11-28
category: "Book"
...
1. 각 샘플에서 작은 거리인 *ϵ* 안에 샘플이 몇 개 놓여 있는지 센다. 이 지역을 샘플의 *ϵ*이웃 이라고 한다.
2. 자기 자신을 포함해 *ϵ*이웃 내에 적어도 `min_samples` 개의 샘플이 있다면, 이를 핵심 샘플로 간주한다. 즉, 핵심 샘플은 밀집된 지역에 있는 샘플이다.
...
DBSCAN은 매우 간단하고 강력하지만, cluster 간 밀집도가 크게 다르거나 cluster 주변에 충분한 저밀도 구역이 없는 경우, 대규모 dataset이라면 잘 작동하지 않는다(계산 복잡도가 *O*(*m*2*n*) 이다).
