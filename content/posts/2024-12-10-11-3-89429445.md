...
description: "표준적인 경사하강법은 경사가 완만할 때는 작은 스텝으로, 경사가 가파를 때는 큰 스텝으로 이동하지만, 속도 자체가 높아지지는 않는다."
date: 2024-12-10
category: "Book"
...
반면에 모멘텀 최적화(momentum optimization)의 경우, gradient를 속도가 아닌 가속도로 사용한다. 즉, 이전 gradient가 얼마였는지를 중요하게 생각하여, 다음과 같이 가중치를 갱신한다 ($\bf{m}$ 은 모멘텀 벡터, *β* 는 모멘텀).

1. $\bf{m} \leftarrow \beta \bf{m} - \eta \nabla_\theta J(\bf{\theta})$
2. $\bf{\theta} \leftarrow \bf{\theta} + \bf{m}$

*β* 는 0과 1사이의 값으로 정해지며, 보통 0.9이다.
...
1에서는 gradient의 제곱을 누적한다. 즉, 비용 함수가 i번째 차원을 따라 가파르다면 *si* 는 반복이 진행되면서 점차 커질 것이다.

2에서는 경사 하강법과 거의 같은 과정이다. 다른 점이 있다면 $\sqrt{\bf{s} + \epsilon}$ 로 나누어 스케일을 조정한다는 것이다. (⊘ 는 원소별 나눗셈을 의미한다.)
...
*β*1 은 모멘텀 최적화의 *β* (모멘텀 감쇠), *β*2 는 RMSprop의 *ρ* (스케일 감쇠)에 해당한다. 보통 *β*1 = 0.9, *β*2 = 0.999 로 초기화하는 경우가 많다.
...
Adam의 단계 5를 간단히 생각하면, $\bf{s}$ 의 제곱근, 즉 *l*2 norm으로 파라미터 업데이트의 스케일을 낮춘다고 생각할 수 있다.

AdaMax는 Adam에서의 *l*2 norm 대신 *l*∞ norm을 사용한다.
...
2. 지수 기반 스케줄링 (Exponential Scheduling)
    ...
    학습률은 *η*(*t*) = *η*00.1*t*/*s* 이다. s 스텝 이후에 10배씩 감소한다.
...
4. 성능 기반 스케줄링 (Performance Scheduling)
    ...
    N 스텝마다 검증 오차를 측정하고, 오차가 줄어들지 않으면 *λ* 배 만큼 학습률을 감소시킨다.
