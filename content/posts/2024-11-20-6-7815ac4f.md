...
description: "결정 트리(Decision Tree)는 분류, 회귀 다중 출력 작업 모두에 사용할 수 있다. 또한 매우 복잡한 데이터셋도 학습할 수 있다."
date: 2024-11-20
category: "Book"
...
결정 트리 예측은 *O*(log2(*m*)) 개의 노드를 거쳐야 한다. 예측에 필요한 전체 복잡도가 특성 수와 무관한 것이다. 따라서 큰 train set에서도 빠르게 예측할 수 있다. 한편, 훈련의 복잡도는 *O*(*n* × *m*log2(*m*))이다.
...
- *G* 는 i번째 노드의 지니 불순도이다.
    
    *i*
    
- *p* 는 i번째 노드에 속한 훈련 샘플 중 클래스 k에 속한 샘플의 비율이다.
...
1. train set를 하나의 특성 k의 임곗값 *tk* 를 사용해 두 개의 subset으로 나눔 (e.g. 꽃잎의 길이 ≤ 2.45cm).
    
    이때, k와 *tk* 는 다음 비용 함수를 최소화하는 값으로 고른다. 아래 비용 함수를 최소화하는 것은 곧 크기에 따른 가중치가 적용된 가장 순수한 subset을 찾는 것이다.
