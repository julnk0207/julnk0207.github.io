...
description: "The basic layer of multi-head attention layer is a scaled dot-product attention layer. It is same as Luong Attention, excluding a scaling parameter."
date: 2025-01-09
category: "Book"
...
- Q: A matrix including a query for a row. Shape is [*n*, *d*].
    
    *queries*
    
    *keys*
    
- K: A matrix including a key for a row. Shape is [*n*, *d*].
    
    *keys*
    
    *keys*
    
- V: A matrix including a value for a row. Shape is [*n*, *d*].
    
    *keys*
    
    *values*
    
- *QK*: One similarity score for each query/key pair.
