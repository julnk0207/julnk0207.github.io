---
title: "[Python for ML] 1. Pythonic Code - 1"
description: "파이썬은 다른 언어과 차별화되는 독특한 스타일을 가지고 있다. 그리고 그 스타일 덕분에 쉽고 직관적인 코드 작성이 가능하다."
date: 2024-04-28
...
다른 점이 보이는가? 일단 결과가 2차원의 리스트이다. 원리를 설명하면, 먼저 바깥에 있는 `word_1`을 고정(fix)시킨 뒤, 내부에 있는 for문을 돌리는 것이다.
