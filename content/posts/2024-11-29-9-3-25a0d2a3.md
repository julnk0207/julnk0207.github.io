...
description: "가우스 혼합 모델(Gaussian Mixture Model, GMM)은 샘플이 파라미터가 알려지지 않은 여러 개의 혼합된 가우스 모델에서 생성되었다고 가정하는 확률 모델이다. 하나의 가우스 분포에서 생성된 모든 샘플은 하나의 cluster를 형성하며, 일반적으로 타원형이다."
date: 2024-11-29
category: "Book"
...
m은 샘플의 개수, p는 모델이 학습할 파라미터의 개수, ℒ 은 모델의 가능도 함수(likelihood function)의 최댓값이다.
