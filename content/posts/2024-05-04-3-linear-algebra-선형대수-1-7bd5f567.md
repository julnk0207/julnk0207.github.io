---
title: "[Python for ML] 3. Linear Algebra (선형대수) - 1"
description: "벡터를 파이썬으로 표현하는 다양한 방법이 있다. 리스트를 쓸 수도 있고, 튜플을 쓸 수도 있고, 딕셔너리를 사용할 수도 있다."
date: 2024-05-04
...
가장 기본적인 행렬 곱셈 방식(행 × 열)을 구현하였다. 코드가 길지만 가장 집중해야할 포인트는 ’행으로 표현된 `matrix_b`에서 어떻게 열을 추출하는가?이다. 그냥 반복문으로 구현했으면 머리가 터질 부분을 zip과 `*`가 완벽하게 구현하는 것(`for column_b in zip(*matrix_b)`)을 확인할 수 있다.
