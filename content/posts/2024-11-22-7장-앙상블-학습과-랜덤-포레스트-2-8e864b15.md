...
description: "부스팅(boosting)은 약한 학습기를 여러 개 연결해서 강한 학습기를 만드는 것을 의미한다."
date: 2024-11-22
category: "Book"
...
(*ŷj*(*i*) 는 i번째 샘플에 대한 j번째 예측기의 예측)

위 오류율을 가지고 예측기 가중치를 구할 수 있다. 예측기가 정확할수록 가중치는 높아진다. *η* 는 학습률 파라미터이다(기본값 1).
...
Adaboost에서 예측을 할 때는 단순히 모든 예측기의 예측을 계산하고, 예측기 가중치 *αj* 를 더해 예측 결과를 만든다. 가중치 합이 가장 큰 클래스가 예측 결과가 된다.
...
- 계산 복잡도는 *O*(*b* × *m*) (b는 구간의 개수, m은 훈련 샘플의 개수)이다. 일반 GBRT보다 수백 배 빠른 셈이다.
