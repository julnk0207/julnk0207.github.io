---
title: "[[MIT] Introduction to Computational Thinking and Data Science] Chapter 9. Understanding Experimental Data"
description: "실험을 통해 얻은 데이터를 이용해 계산을 설계해야 할 때가 있다. 예를 들어, 용수철의 용수철 상수 k의 값을 알아내기 위해 무게를 늘려가며 용수철이 늘어나는 길이를 재는 실험을 생각해볼 수 있다. cf. 용수철에 힘을 주어 변형을 가하면 아래와 같은 ’훅의 법칙’을 따른다."
date: 2023-02-18
...
운이 좋게도, 파이썬에서는 일일이 이를 계산할 필요가 없다.
...
을 사용하면 원하는 차수에 대해 가장 잘 들어맞는 다항식을 찾아준다.
...
*yi*는 측정 값, *pi*는 추정 값, *μ*는 측정 값을 평균을 의미한다. 추정 오차를 측정 데이터의 변이성(편차)으로 나누는 과정으로, 데이터 변이성의 비율을 알려준다.
