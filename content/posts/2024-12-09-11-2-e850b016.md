...
description: "배치 정규화(Batch Normalization)를 이용하면 vanishing gradient와 exploding gradient 문제를 해결할 수 있다."
date: 2024-12-09
category: "Book"
...
- *ϵ* 은 분모가 0이 되는 것을 막기 위한 작은 수이다.
- *x̂* 는 평균이 0이고 정규화된 샘플 i의 입력이다.
    
    (*i*)
    
- *γ*: 층의 출력 스케일 파라미터 벡터.
- *β*: 층의 출력 이동 파라미터 벡터.
- *z*: 배치 정규화 연산의 출력. 입력의 스케일을 조정하고 이동시킨 결과.
